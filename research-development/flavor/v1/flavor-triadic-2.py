...
def derive_sector_triads() -> dict:
    ...
    We use the following mapping (purely from symbolic semantics):

      up-type quarks:
        triad [1,2,3] = Unity / Duality / Trinity
        → "fundamental tone" → building-block sector

      down-type quarks:
        triad [2,4,6] = Duality / Foundation / Balance
        → structure & balance → complements up

      charged leptons:
        triad [3,6,9] = Trinity / Hexagon / Triple Trinity
        → completion & stability in the lepton sector

      neutrinos:
        triad [12,24,36] = Zodiac / Hours in Day / Double Zodiac
        → time cycles, macro cycles → oscillatory/cosmic sector

    This choice is fixed and not tuned to data.
    """
    df = get_base60_table()
...
def exponents_from_triads(tri: list, allowed_digits: list) -> list:
    """
    Global harmonic rule mapping Base-60 triad digits to eps-exponents.

    Rule:
      - Define a 'harmonic depth' index hd(d) for each allowed digit,
        using its position in the sorted allowed list.
      - For a triad (d1,d2,d3) with d3 = max digit, define exponents:

          e_g = hd(d_max) - hd(d_g)

        so that the heaviest generation (largest digit) has exponent 0,
        and lighter generations have positive integer exponents
        reflecting how 'deep' they sit in the harmonic ladder.

    This is a single, sector-independent mapping.
    """
...
def sector_generation_patterns(cfg: Config):
    eps = cfg.eps
    return {
        "up":      [eps**4, eps**2, 1.0],
        "down":    [eps**3, eps,    1.0],
        "lepton":  [eps**3, eps,    1.0],
        "neutrino":[eps,    1.0,    1.0],
    }
...
def generate_proto_matrices(cfg: Config):
    ...
    patterns = sector_generation_patterns(cfg)
...
def triad_heavy_basis(Nh=6):
    ...
    This triad still satisfies the triadic closure rule, but admits much
    more democratic heavy structures → larger PMNS mixing.
    """
    ks = np.array([0, 1, 2])
    i = np.arange(Nh)
    basis = []
...
def run_pipeline(seed: int,
    ...
    Full pipeline with harmonically derived, sector-dependent proto structure:
      - build alignment kernel on 9 sites (D_360 law)
      - derive sector triads from Base-60 symbolic meanings
      - map triads to generation patterns via a universal exponent rule
    ...
    # 2. proto (sector-dependent triads → generation patterns → 9-site scales)
