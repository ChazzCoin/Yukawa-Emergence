...
class Config:
    ...
    mu0 = 1.0e12     # GeV
    mu_EW = 91.1876  # GeV
    Lambda_Maj = 7.0e13 #1.0e14  # GeV (overall heavy Majorana scale)
    ...
    lam = 0.13
    # Optional: interpret κ as κ(t0) = exp(-λ t0)
    lambda_align = 1.0  # arbitrary units
    t_align = -np.log(eps) / lambda_align
...
def build_alignment_kernel_from_kappa(kappa: float,
    ...
    This is the discrete realization of the semigroup profile
    k(d) = κ^d with a single forbidden distance d_star (A4).
    """
    kappa = float(kappa)
    K = np.zeros((N, N), dtype=float)
    for i in range(N):
                ...
                for ks in candidate_ks:
                    ...
                    for seed in range(10):
                        res = run_pipeline(
                            ...
                            triad_ks=ks
                            # and pass exponents_* into generate_proto_matrices
                        )
                        chi2_best = min(chi2_best, res["chi2"])
                    print(exp_d, exp_e, exp_nu, ks, "→ best χ²", chi2_best)
