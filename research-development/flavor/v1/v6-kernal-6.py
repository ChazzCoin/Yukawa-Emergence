...
KAPPA = 0.242
EPS_ALIGN = 1.0 / KAPPA
# Dynamically controlled coherence parameter κ (overwritten by optimizer)
def set_kappa(kappa_val):
    """Set global KAPPA and EPS_ALIGN safely."""
    global KAPPA, EPS_ALIGN
    # clamp kappa to meaningful physical region
    KAPPA = float(np.clip(kappa_val, 0.01, 10.0))
    EPS_ALIGN = 4.13
...
def unpack_params(X):
    """
    32-dimensional parameter vector X ->
      (Δu, Δd, Δe, Δnu,
...
def params_cost_vectorized(X,
    ...
    # NEW: apply κ globally before computing Yukawas
    set_kappa(kappa_opt)
...
def optimize_params_CMA(num_restarts=6,
    ...
    for r in range(num_restarts):
        ...
        X0[31] = 0.3  # initial κ guess (≈ your discovered optimum)
...
if __name__ == "__main__":
    ...
    # apply chosen κ for final evaluation
    set_kappa(kappa_opt)
