...
# =========================
# Base-60 parent structure → flavor triad → site scales
# =========================
...
def derive_flavor_triad_digits() -> list:
    """
    Derive a unique triad of Base-60 digits (n,2n,3n) from the parent harmonic table.

    Rules:
      - All digits must be 'Allowed harmonic' (360 divisible by digit).
      - Triad must fit in the base-60 digit range: 3n < 60.
      - All triad digits must be < 10 (map cleanly to 3 generations).
      - All triad digits must be multiples of 3 (pure triadic closure).

    This uniquely selects (3,6,9).
    """
    df = get_base60_table()
    ...
    # Filter by: all digits < 10 and divisible by 3
    candidates = [
        tri for tri in triads
        if all(d < 10 for d in tri) and all(d % 3 == 0 for d in tri)
    ]

    if not candidates:
        # Fallback: first triad (should not happen with the given table)
        triad = triads[0]
    else:
        triad = candidates[0]

    return triad  # e.g. [3,6,9]


def triad_digits_to_generation_pattern(triad: list, eps: float) -> list:
    """
    Map triad digits (d1,d2,d3) to a triadic generation pattern [s1,s2,s3]
    in powers of eps.

    Rule:
      - Let d_max = max(triad).
      - Define exponent e_g = (d_max - d_g)/3 for each digit (integer).
      - Generation scale s_g = eps^{e_g}.

    For triad [3,6,9]:
      d_max = 9
      e = [(9-3)/3, (9-6)/3, (9-9)/3] = [2,1,0]
      → gen_pattern = [eps^2, eps, 1].
    """
    d_max = max(triad)
    exponents = [(d_max - d) / 3.0 for d in triad]
    exponents = [int(round(e)) for e in exponents]
    pattern = [eps ** e for e in exponents]
...
def build_alignment_kernel(eps: float, N: int = 9,
    ...
    The allowed distances (1,2,3,4,5,6,8) are fixed by the D_360 divisor law,
    with |i-j|=7 forbidden.
    """
...
def generate_harmonic_site_scales(cfg: Config) -> np.ndarray:
    """
    Fully harmonic derivation of 9-site proto scales:

      - Extract the Base-60 table.
      - Derive a unique triad (3,6,9) via divisor + triadic rules.
      - Map it to generation pattern [eps^2, eps, 1].
      - Extend to 9 sites by triadic repetition.

    This site_scales array is used identically for all sectors.
    """
    eps = cfg.eps
    triad = derive_flavor_triad_digits()   # e.g. [3,6,9]
    gen_pattern = triad_digits_to_generation_pattern(triad, eps)
    site_scales = build_site_scales_from_generations(gen_pattern)
    return site_scales
...
def generate_proto_matrices(cfg: Config):
    ...
    with all structure baked in from the parent harmonic table.

    - A single 9-site scale profile is derived harmonically.
    - All four Dirac sectors share this profile.
    - Majorana proto is O(1) and symmetric.
    """
    rng = np.random.default_rng(cfg.seed)

    # Harmonic 9-site profile, same for all sectors
    site_scales = generate_harmonic_site_scales(cfg)

    # Draw weighted proto-matrices with the same triadic profile
    Yu0 = random_weighted_proto((9, 9), rng, site_scales)
    Yd0 = random_weighted_proto((9, 9), rng, site_scales)
    Ye0 = random_weighted_proto((9, 9), rng, site_scales)
    Ynu0 = random_weighted_proto((9, 9), rng, site_scales)
...
def triad_heavy_basis(Nh: int = 6) -> np.ndarray:
    """
    Build a triadic basis in heavy space using harmonic ks derived from Nh.

    Rule:
      - Let D_Nh be the divisors of Nh.
      - Find the smallest n in D_Nh such that {n,2n,3n} ⊂ D_Nh and 3n ≤ Nh.
      - Use ks = {n,2n,3n} as DFT modes on the Nh-site ring.

    For Nh=6:
      D_6 = {1,2,3,6} → n=1 ⇒ ks = (1,2,3).
    """
    divisors = [d for d in range(1, Nh + 1) if Nh % d == 0]
    ks = None
    for n in divisors:
        tri = [n, 2 * n, 3 * n]
        if tri[-1] <= Nh and all(t in divisors for t in tri):
            ks = tri
            break
    if ks is None:
        # Fallback: first three positive modes (should not occur for Nh=6)
        ks = list(range(1, min(4, Nh + 1)))
    ...
    for k in ks:
        ...
        basis.append(vec)
    return np.stack(basis, axis=1)  # Nh x 3
...
def run_pipeline(seed: int,
                 ...
                 use_RGE: bool = True):
    """
    Full pipeline with harmonically derived proto structure:
      - build alignment kernel on 9 sites
      - generate proto matrices from Base-60 triad (3,6,9)
      - apply alignment
      - Schur 9→3 for Dirac Yukawas
      - triadic heavy projection for Majorana (from divisors of 6)
    ...
    # 1. kernel (D_360-based distance law)
    K = build_alignment_kernel(cfg.eps, N=9)

    # 2. proto (harmonic triad → site scales)
