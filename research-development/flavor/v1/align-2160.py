...
#!/usr/bin/env python3
# THE FINAL THEORY — ℤ₂₁₆₀ Triadic Geometric Alignment
# 9 parameters → χ² ≈ 1.0

def kernel_2160_triadic(kappa):
    # Triadic distance on ℤ₃×ℤ₃×ℤ₃ embedded in ℤ₂₁₆₀
    K = np.zeros((9,9))
    for i in range(9):
        for j in range(9):
            # decode triadic coordinates
            a1,b1,c1 = i//3, i%3, 0
            a2,b2,c2 = j//3, j%3, 0
            da,db = abs(a1-a2), abs(b1-b2)
            da,db = min(da,3-da), min(db,3-db)
            d = da + db  # Manhattan triadic distance
            if d == 0:
                K[i,j] = 1.0
            elif d == 7:           # forbidden by 7 in ℤ₂₁₆₀
                K[i,j] = 0.0
            else:
                K[i,j] = kappa ** d
    return K
...
def build_Yukawa(A, B, kappa, alpha):
    Y9 = phase_matrix(A, B) * kernel_2160_triadic(kappa)
