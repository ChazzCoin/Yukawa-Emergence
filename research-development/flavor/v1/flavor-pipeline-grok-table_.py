...
class Config:
    ...
    t0 = np.log(mu0)
    t1 = np.log(mu_EW)
    dt = -0.01  # Increased step size for numerical stability (was -0.001)
...
def beta_Yukawas(Yu, Yd, Ye, Ynu, g1, g2, g3):
    # Safeguard: Return zero betas if inputs contain NaN or Inf to prevent propagation
    if np.any(np.isnan([Yu, Yd, Ye, Ynu])) or np.any(np.isinf([Yu, Yd, Ye, Ynu])):
        return np.zeros_like(Yu), np.zeros_like(Yd), np.zeros_like(Ye), np.zeros_like(Ynu)
...
def beta_kappa_L(kappa_L, Yu, Ye, g2, lam):
    ...
    """
    # Safeguard: Return zero if input contains NaN or Inf
    if np.any(np.isnan(kappa_L)) or np.any(np.isinf(kappa_L)) or np.any(np.isnan(Yu)) or np.any(np.isnan(Ye)):
...
def run_RGE_full(Yu0, Yd0, Ye0, Ynu0, kappa_L0, g1_const, g2_const, g3_const, cfg: Config):
    ...
    while (cfg.dt < 0 and t > cfg.t1) or (cfg.dt > 0 and t < cfg.t1):
        step += 1
        # Optional: Monitor progress every 200 steps (remove for production)
        # if step % 200 == 0:
        #     print(f"RGE step {step}, t={t:.2f}")
        ...
        if np.any(np.isnan([Yu, Yd, Ye, Ynu, kappa_L])) or np.any(np.isinf([Yu, Yd, Ye, Ynu, kappa_L])):
...
Best seed: 6
chi2 = 445.6502502247696
Up masses (GeV):    [1.73000000e+02 3.84364928e-01 2.19679173e-03]
Down masses (GeV):  [4.18000000e+00 5.13652881e-01 6.72226698e-04]
Lepton masses (GeV): [1.77700000e+00 4.09707785e-02 3.63837179e-04]
Neutrino masses (GeV): [1.07548621e-10 3.77414302e-12 4.75908515e-13]
Δm² (eV²): (np.float64(1.4017666599668046e-05), np.float64(0.011566479455723299))
CKM angles (rad): (np.float64(0.11779949542937702), np.float64(0.024537244666078453), np.float64(0.001760241026735564)) δq: -0.12974536012151536
PMNS angles (rad): (np.float64(0.4118286711689401), np.float64(0.17879096355900262), np.float64(0.0497801670732157)) δℓ: -1.4416352263861982

=== χ² breakdown for best seed ===
m_c/m_t               th =  2.2218e-03,  exp =  7.0000e-03,  sigma =  3.5000e-03,  pull = -1.37,  chi2_i =  1.86
m_u/m_t               th =  1.2698e-05,  exp =  1.0000e-05,  sigma =  5.0000e-06,  pull =  0.54,  chi2_i =  0.29
m_s/m_b               th =  1.2288e-01,  exp =  2.0000e-02,  sigma =  1.0000e-02,  pull =  10.29,  chi2_i =  105.85
m_d/m_b               th =  1.6082e-04,  exp =  1.0000e-03,  sigma =  5.0000e-04,  pull = -1.68,  chi2_i =  2.82
m_mu/m_tau            th =  2.3056e-02,  exp =  6.0000e-02,  sigma =  3.0000e-02,  pull = -1.23,  chi2_i =  1.52
m_e/m_tau             th =  2.0475e-04,  exp =  3.0000e-04,  sigma =  1.5000e-04,  pull = -0.64,  chi2_i =  0.40
theta12_q (rad)       th =  1.1780e-01,  exp =  2.2600e-01,  sigma =  2.2600e-02,  pull = -4.79,  chi2_i =  22.92
theta23_q (rad)       th =  2.4537e-02,  exp =  4.1000e-02,  sigma =  4.1000e-03,  pull = -4.02,  chi2_i =  16.12
theta13_q (rad)       th =  1.7602e-03,  exp =  3.5000e-03,  sigma =  3.5000e-04,  pull = -4.97,  chi2_i =  24.71
theta12_l (rad)       th =  4.1183e-01,  exp =  5.9000e-01,  sigma =  5.9000e-02,  pull = -3.02,  chi2_i =  9.12
theta23_l (rad)       th =  1.7879e-01,  exp =  8.4000e-01,  sigma =  8.4000e-02,  pull = -7.87,  chi2_i =  61.96
theta13_l (rad)       th =  4.9780e-02,  exp =  1.5000e-01,  sigma =  1.5000e-02,  pull = -6.68,  chi2_i =  44.64
Delta m2_21 (eV^2)    th =  1.4018e-05,  exp =  7.4000e-05,  sigma =  2.2200e-05,  pull = -2.70,  chi2_i =  7.30
Delta m2_31 (eV^2)    th =  1.1566e-02,  exp =  2.5000e-03,  sigma =  7.5000e-04,  pull =  12.09,  chi2_i =  146.14
"""