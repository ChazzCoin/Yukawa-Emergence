...
# ============================================================
# Harmonic, fully aligned pipeline with distance-7 gap
# Parent on Z_360 -> S^ -> embedding search -> holographic Yukawas
# -> simple seesaw + mixing
# ============================================================
...
def rescale_distances(D, max_scale=8.0):
    """
    Compress raw distances to a small band to get richer structure:
        d_scaled = (d / d_max) * max_scale
    """
    d_max = np.max(D)
    if d_max == 0:
...
# ---------------------------------------------------
# 5. Holographic map with explicit d = 7 gap
# ---------------------------------------------------
...
def holographic_kernel_with_gap(dist_matrix, lambd, gap_distance=7.0, tol=1e-6):
    ...
    We treat distances within 'tol' of the gap as exactly forbidden.
    """
...
def build_all_sectors(freqs, amps, dist_matrix_scaled, gap_distance=7.0):
    sectors = {}

    # Decay constants now compatible with scaled distances in [0, ~8]
    lambd_u = 0.6
    lambd_d = 0.45
    lambd_e = 0.5
    lambd_nu = 0.15
    ...
    sectors["up"] = build_sector_yukawa(freqs, amps, dist_matrix_scaled,
                                        lambd_u, 0.0, gap_distance)
    sectors["down"] = build_sector_yukawa(freqs, amps, dist_matrix_scaled,
                                          lambd_d, np.pi / 6.0, gap_distance)
    sectors["charged_lepton"] = build_sector_yukawa(freqs, amps, dist_matrix_scaled,
                                                    lambd_e, np.pi / 3.0, gap_distance)
    sectors["neutrino_D"] = build_sector_yukawa(freqs, amps, dist_matrix_scaled,
                                                lambd_nu, np.pi / 2.0, gap_distance)
    return sectors


# ---------------------------------------------------
# 6. Simple seesaw + mixing
# ---------------------------------------------------

def build_majorana_matrix(dist_matrix_scaled, lambd_M=0.3, gap_distance=7.0):
    """
    Heavy Majorana matrix M_R with same geometric law + gap.
    """
    K = holographic_kernel_with_gap(dist_matrix_scaled, lambd_M, gap_distance)
...
def seesaw_light_neutrinos(Y_nu, M_R, v=1.0):
    ...
    # Make Hermitian by symmetrizing small numeric asymmetries
...
def run_pipeline():
    ...
    sectors = build_all_sectors(freqs_sel, amps_sel, D_scaled, gap_distance=7.0)
    ...
    M_R = build_majorana_matrix(D_scaled, lambd_M=0.25, gap_distance=7.0)
    ...
    # Diagonalize charged leptons and neutrinos (3x3 light block as toy)
    print("=== Toy 3x3 mixing from charged lepton and neutrino sectors ===")
    # Use upper-left 3x3 as the "light" block
    Y_e = sectors["charged_lepton"][:3, :3]
    Y_nu_light = Y_nu[:3, :3]

    # Hermitian combinations
    H_e = Y_e.conj().T @ Y_e
    H_nu = m_nu[:3, :3]  # already Hermitian
