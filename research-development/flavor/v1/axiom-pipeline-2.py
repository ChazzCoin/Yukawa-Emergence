...
# =========================
# Alignment semigroup (A6′)
# =========================

class AlignmentSemigroup:
    """
    One-parameter Toeplitz Schur semigroup with decay base q(t) = exp(-λ t) ∈ (0,1].
    """
    def __init__(self, lam: float, d_star: int = 7, N: int = 9):
        self.lam = float(lam)
        self.d_star = int(d_star)
        self.N = int(N)

    def decay(self, t: float) -> float:
        """q(t) = exp(-λ t) ∈ (0,1] for t ≥ 0."""
        return float(np.exp(-self.lam * t))

    def kernel(self, t: float) -> np.ndarray:
        """
        K_t[i,j] = q(t)^{|i-j|}  for |i-j| ≠ d_star, |i-j|>0
                 = 1            for |i-j| = 0
                 = 0            for |i-j| = d_star.
        """
        q = self.decay(t)
...
def build_alignment_kernel_from_kappa(kappa: float,
                                      N: int = 9,
                                      d_star: int = 7) -> np.ndarray:
    """
    Build the 9x9 alignment kernel for a *fixed* κ:
      K_ij = κ^{|i-j|} for |i-j| ≠ d_star, |i-j|>0
      K_ii = 1,
      K_ij = 0 for |i-j| = d_star.
    """
    kappa = float(kappa)
    K = np.zeros((N, N), dtype=float)
    for i in range(N):
...
def build_alignment_kernel(eps: float, N: int = 9, d_star: int = 7) -> np.ndarray:
    """
    Build the 9x9 alignment kernel with a *decaying* profile:

      K_ij = eps^{|i-j|}  for |i-j| ≠ d_star, |i-j| > 0
           = 1           for |i-j| = 0
           = 0           for |i-j| = d_star.

    Here 0 < eps < 1 is the alignment decay factor e^{-λ t0}.
    This realizes:
      - A1: K depends only on the discrete distance d = |i-j|
      - A2: real, symmetric, non-negative, K_ii = 1
      - A4: single forbidden distance d_star = 7
      - A6′ + Thm 3.x: k(d) = eps^d with a single scale eps.
    """
    eps = float(eps)
    K = np.zeros((N, N), dtype=float)
    for i in range(N):
...
def generation_pattern(eps, exponents):
    """
    exponents = (a, b, c) with c typically 0.
    Returns [eps**a, eps**b, eps**c], with eps in (0,1).
    """
    a, b, c = exponents
    return [eps ** a, eps ** b, eps ** c]
...
def generate_proto_matrices(cfg: Config,
    ...
    exponents_* specify generation patterns via eps**a, eps**b, eps**c.
    """
...
def run_pipeline(seed: int,
    ...
    # Axiom checks (debug / development)
    assert np.allclose(K, K.T), "K must be symmetric (A2)"
    assert np.allclose(np.diag(K), 1.0), "Diagonal entries must be 1 (A2)"
    assert np.all(K >= 0.0), "K must be non-negative (A2)"

    # Single-gap check (A4): all distances 1..8 allowed except 7
    N = K.shape[0]
    distances_seen = set()
    for i in range(N):
        for j in range(N):
            d = abs(i - j)
            if d > 0 and K[i, j] != 0.0:
                distances_seen.add(d)
    assert 7 not in distances_seen, "Distance 7 must be forbidden (A4)"
    assert distances_seen.issuperset({1, 2, 3, 4, 5, 6, 8}), "All other distances should appear (A4)"

    # 2. proto (now depends on exponents_*)
    Yu0, Yd0, Ye0, Ynu0, M0 = generate_proto_matrices(
        cfg, use_site_hierarchy,
...
def simple_seed_scan():
    ...
    for seed in range(N_seeds):
        res = run_pipeline(seed, cfg,
                           use_RGE=True,
                           use_site_hierarchy=True)
        chi2_vals.append(res["chi2"])
        all_results.append(res)
        print(f"seed {seed}: chi2 = {res['chi2']:.3g}")

    best_idx = int(np.argmin(chi2_vals))
    best = all_results[best_idx]

    print("\nBest seed in simple scan:", best_idx)
...
candidate_exponents_d = [(2, 1, 0), (3, 1, 0)]
candidate_exponents_e = [(4, 2, 0), (3, 1, 0)]
candidate_exponents_nu = [(1, 0, 0), (2, 0, 0)]
candidate_ks = [(1, 2, 3), (1, 2, 4)]
...
def structural_scan():
    ...
    # Structural scan over (exponents_d, exponents_e, exponents_nu, triad_ks)
    for exp_d in candidate_exponents_d:
        for exp_e in candidate_exponents_e:
            for exp_nu in candidate_exponents_nu:
                for ks in candidate_ks:
                    ...
                    for seed in range(N_seeds):
                        res = run_pipeline(
                            ...
                            use_site_hierarchy=True,
                            triad_ks=ks,
                            exponents_u=(4, 2, 0),   # keep up-sector fixed
                            exponents_d=exp_d,
                            exponents_e=exp_e,
                            exponents_nu=exp_nu,
                        )
                        if res["chi2"] < chi2_best:
                            chi2_best = res["chi2"]
                            best_res = res
                            best_seed = seed

                    print(exp_d, exp_e, exp_nu, ks,
                          f"→ best seed {best_seed}, best χ² {chi2_best:.3g}")

                    if chi2_best < global_best_chi2:
                        global_best_chi2 = chi2_best
                        global_best_res = best_res
                        global_best_info = (exp_d, exp_e, exp_nu, ks, best_seed)

    if global_best_res is None:
        print("No successful runs (all χ² were inf or NaN).")
    else:
        best = global_best_res
        exp_d, exp_e, exp_nu, ks, best_seed = global_best_info

        print("\n=== Global best configuration ===")
        print("exponents_d =", exp_d,
              "exponents_e =", exp_e,
              "exponents_nu =", exp_nu,
              "triad_ks =", ks,
              "seed =", best_seed)
        ...
        # Detailed χ² breakdown for the global best
        print("\n=== χ² breakdown for global best ===")
        breakdown = chi2_breakdown(best)
        for entry in breakdown:
            name = entry["name"]
            th = entry["theory"]
