...
def emergent_alpha(lam_triad, eps=1e-6):
    """
    Emergent kernel steepness derived from the triad itself.
    α ~ 1 / Var(log λ)
    """
...
# ----------------------------------------------------------------------
def demonstrate_alignment_mechanism():
    """
    Direct numerical demonstration of the paper's alignment mechanism.
    Assume realistic charged-lepton and neutrino Hermitian operators
    (diagonal in their own bases, with m_μ ≈ m_τ in neutrino sector for degeneracy).
    """
    # Example realistic masses squared (normalized)
    m2_e = np.array([1.0, 100.0, 10000.0])  # rough e : μ : τ
    m2_nu = np.array([0.0, 1.0, 1.05])      # nearly degenerate μ-τ, small solar splitting

    He = np.diag(m2_e)      # charged-lepton basis
    Hnu = np.diag(m2_nu)    # initial neutrino basis

    # Alignment energy as function of μ-τ rotation θ
    ...
    for theta in thetas:
        ...
        R = np.array([[1, 0, 0],
                      ...
                      [0, s,  c]])
        Hnu_rot = R.T @ Hnu @ R
        commutator = He @ Hnu_rot - Hnu_rot @ He
        E = np.linalg.norm(commutator, 'fro')**2   # Frobenius norm
        energies.append(E)

        # Compute resulting θ23 (assuming electron direction fixed)
        # Simple: θ23 ≈ angle between μ-τ block
        U_pmns_abs = np.abs(np.eye(3) @ R)  # trivial Ue, Uν = R in 2-3
        s23 = U_pmns_abs[1,2]
        theta23_vals.append(np.arcsin(min(1.0, s23)))

    energies = np.array(energies)
    theta_min = thetas[np.argmin(energies)]

    print(f"Unique minimum at θ* ≈ {theta_min:.3f} rad")
    print(f"Alignment energy at min: {np.min(energies):.3e} (should be near 0)")
    print(f"Resulting atmospheric angle θ23 ≈ {np.arcsin(np.sin(theta_min)):.3f} rad")
...
def main():
    ...
    alpha = emergent_alpha(lam_gen)
    F_base = np.exp(-alpha * lam_gen ** 2)
...
def run_emergence():
    print("\n=== Running Emergent Pipeline ===\n")
    demonstrate_alignment_mechanism()
...
Base kernel F_base(lam_gen): [6.33593107e-05 4.14624599e-15 3.53773567e-18]

=== Yukawa-like mass scales F_s ===
Up-type (F_u):         [1.16046625e-06 1.39091058e-18 3.53773567e-18]
Down-type (F_d):       [4.26911677e-07 2.79371858e-17 3.53773567e-18]
Charged leptons (F_e): [1.16046625e-06 4.14624599e-15 1.76133488e-19]
Neutrino (F_n):        [1.57052029e-07 2.79371858e-17 6.47958890e-20]

Best lepton region permutations:
  pe (e sectors)  = (1, 0, 2)
  pn (nu sectors) = (2, 0, 1)
Best total chi^2  ≈ 4654.37

Mass ratios (m1/m3, m2/m3) from F_s:
mu/mt:     1.199e-12, mc/mt:     3.049e-12
md/mb:     8.287e-12, ms/mb:     6.544e-11
me/mtau:   1.518e-13, mmu/mtau:  3.573e-09
...
mu/mt       : model=1.199e-12, target=2.200e-05, chi2_contrib=586.25
mc/mt       : model=3.049e-12, target=7.500e-03, chi2_contrib=979.89
md/mb       : model=8.287e-12, target=1.100e-03, chi2_contrib=733.15
ms/mb       : model=6.544e-11, target=2.200e-02, chi2_contrib=807.81
me/mtau     : model=1.518e-13, target=2.900e-04, chi2_contrib=957.12
mmu/mtau    : model=3.573e-09, target=5.900e-02, chi2_contrib=578.86
...
Unique minimum at θ* ≈ 0.000 rad
Alignment energy at min: 0.000e+00 (should be near 0)
Resulting atmospheric angle θ23 ≈ 0.000 rad
